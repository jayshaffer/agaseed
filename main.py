...
open_tower_container = ["open_tower", "Open Tower", open_tower_dropdown, open_tower_options]
...
world_state_container = ["world_state", "World State", world_state_dropdown, world_state_options]
...
boss_shuffle_container = ["boss_shuffle", "Boss Shuffle", boss_shuffle_dropdown, boss_shuffle_options]
...
enemy_damage_container = ["enemy_damage", "Enemy Damage", enemy_damage_dropdown, enemy_damage_options]
...
enemy_health_container = ["enemy_health", "Enemy Health", enemy_health_dropdown, enemy_health_options]
...
class GenerateSeedButton(Button):
    ...
    async def generate_seed(self):
        ...
        await seed.create_patched_game(
            ...
            heartspeed='off',
            heartcolor='red',
            spritename=sprites[random.randint(0, len(sprites) - 1)]['name'], #can be any sprite listed at https://alttpr.com/sprites
...
class Dropdowns(BoxLayout):
    def __init__(self, key, name, options, defaultOption, *args, **kwargs):
        ...
        dropdown.bind(on_select=lambda instance, text: self.handle_dropdown_click(button, text))
        ...
        for option in options:
            ...
            option_button = Button(
                    ...
                    border=(1, 1, 1, 1),
                    text=option_text,
                    on_release=lambda btn, text=option_text, value=option_value: self.handle_option_click(dropdown, text, value),
                )
...
class AgaseedApp(App):
    def build(self):
        ...
        layout = BoxLayout(orientation='vertical', spacing=10, size_hint_y=None, size=(Window.width, Window.height))
        layout.add_widget(Label(text="Item Placement", halign="left", size_hint_y=None, height=50, color=(0, 0, 0, 1)))
        layout.add_widget(Dropdowns("glitches", "Glitches", glitches_options, "None"))
        layout.add_widget(Dropdowns("item_placement", "Item Placement", item_placement_options, "Advanced"))
        layout.add_widget(Dropdowns("shuffle", "Shuffle", shuffle_options, "Standard"))
        layout.add_widget(Dropdowns("accessibility", "Accessibility", accessibility_options, "Inventory"))
        layout.add_widget(Label(text="Goal", halign="left", size_hint_y=None, height=50, color=(0, 0, 0, 1)))
        layout.add_widget(Dropdowns("goal", "Goal", goal_options, "Ganon"))
        layout.add_widget(Dropdowns("open_tower", "Open Tower", open_tower_options, "7"))
        layout.add_widget(Dropdowns("ganon_vulnerable", "Ganon Vulnerable", ganon_vulnerable_options, "7"))
        layout.add_widget(Label(text="Gameplay", halign="left", size_hint_y=None, height=50, color=(0, 0, 0, 1)))
        layout.add_widget(Dropdowns("world_state", "World State", world_state_options, "Open"))
        layout.add_widget(Dropdowns("entrances", "Entrance Shuffle", entrance_shuffle_options, "None"))
        layout.add_widget(Dropdowns("boss_shuffle", "Boss Shuffle", boss_shuffle_options, "None"))
        layout.add_widget(Dropdowns("enemy_shuffle", "Enemy Shuffle", enemy_shuffle_options, "None"))
        layout.add_widget(Dropdowns("hints", "Hints", hints_options, "On"))
        layout.add_widget(Label(text="Difficulty", halign="left", size_hint_y=None, height=50, color=(0, 0, 0, 1)))
        layout.add_widget(Dropdowns("swords", "Swords", swords_options, "Randomized"))
        layout.add_widget(Dropdowns("item_pool", "Item Pool", item_pool_options, "Normal"))
        layout.add_widget(Dropdowns("item_function", "Item Function", item_function_options, "Normal"))
        layout.add_widget(Dropdowns("enemy_damage", "Enemy Damage", enemy_damage_options, "Default"))
        layout.add_widget(Dropdowns("enemy_health", "Enemy Health", enemy_health_options, "Default"))
        right_layout = BoxLayout(orientation='vertical', spacing=10)
        right_layout.add_widget(GenerateSeedButton(text="Generate Seed", size_hint_y=None, size_hint_x=0.3))
        right_layout.add_widget(Label(text="", halign="left", size_hint_y=None, height=50, color=(0, 0, 0, 1)))
